...
📌 最新功能更新 (New Features)
- 新增會員單一登入：人臉識別會員登入後保留 session，可直接快速點餐。
- 新增會員點數機制：
  - 結帳可自動獲得點數 (每滿 10 元得點數可自訂倍率)。
  - 會員可在結帳時使用點數折抵現金（每 N 點可折抵 1 元）。
  - 管理員可在後台設定點數功能開關、折抵比例與賺點倍數。
- 新增結帳預覽：購物車中顯示小計、折扣、點數折抵、應付金額，並提供確認 modal。
- 新增菜單篩選/排序功能：
  - 顧客可依分類、關鍵字、最高價格篩選。
  - 可按熱門、新品、價格升/降排序。
- 新增訂單管理功能：
  - 管理員可搜尋訂單、按狀態/付款方式/日期範圍篩選。
  - 顯示營收報表與熱門品項排行榜。
  - 管理員可直接編輯訂單內容、更新訂單狀態並儲存。
  - 支援訂單 CSV 匯出。
- 新增後台菜單管理：支援新增/刪除/編輯菜單與圖片上傳、推薦標記、新品標記。

✅ 版本紀錄 (Short Changelog)
- v1.3: 新增會員點數折抵、購物車結帳預覽、後台點數設定開關
- v1.2: 新增訂單內容編輯、狀態管理、CSV 匯出與報表功能
- v1.1: 新增 menu 篩選/排序+熱門推薦、會員折扣
